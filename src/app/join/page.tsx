...
// Import pro Firebase Firestore, předpokládá, že 'db' je exportováno ze src/lib/firebase
import { db } from '../../lib/firebase'; // Upravená cesta!
...
export default function JoinPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Základní validace polí (pokud chybí lokalizace, musíte ji přidat)
      if (!formData.email || !formData.name || !formData.university || !formData.year) {
        throw new Error("Některá povinná pole nejsou vyplněna."); 
      }
      
      const dataToSave = {
        ...formData,
        timestamp: new Date().toISOString(), // Přidání časového razítka
      };

      // Uložení dat do Firebase Firestore do kolekce 'registrace'
      // Zde je klíčové, aby Pravidla Firestore povolovala zápis (viz níže)
      await addDoc(collection(db, "registrace"), dataToSave);

      // Po úspěchu přepněte stav a zobrazte děkovnou obrazovku
      setSubmitted(true);

    } catch (err) {
      console.error("Chyba při odesílání registrace do Firebase:", err);
      // Zobrazení chybové zprávy (form.error.submission_failed)
      setError(t("form.error.submission_failed")); 
    } finally {
      setLoading(false);
    }
  };

  // --- ZOBRAZENÍ PO ODESLÁNÍ (DĚKUJEME) ---
  if (submitted) {
    return (
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <main className="relative z-10 max-w-3xl mx-auto px-6 py-16">
        ...
        {/* Zobrazení chybové zprávy, pokud nastala chyba */}
        {error && (
            <div className="bg-red-900/40 border border-red-500/30 p-4 mb-6 rounded-xl text-red-300 text-center">
                {error}
            </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="bg-gradient-to-br from-purple-900/40 to-black/40 backdrop-blur-sm border border-purple-500/30 hover:border-purple-500/60 transition-all p-8 md:p-10 rounded-2xl space-y-6">
            ...
            {/* Submit Button (s loaderem) */}
            <button
              type="submit"
              disabled={loading} // Zakáže tlačítko při odesílání
              className={`w-full py-4 rounded-lg font-semibold text-lg transition-all duration-300 ${
                loading 
                  ? 'bg-gray-600 cursor-not-allowed' 
                  : 'bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 transform hover:scale-[1.02] glow-purple'
              }`}
            >
